...
class PieceStorage {
public:
    friend class PeerConnect;

    PieceStorage(const TorrentFile& tf, const std::filesystem::path& outputDirectory, size_t toDownload);
...
private:
    ...
    std::size_t piecesInProgressCount_;
    std::size_t pieceLength_;
    mutable std::mutex mutexForQueue_;
    mutable std::mutex mutexForWriting_;
    std::size_t totalPiecesCount_;
