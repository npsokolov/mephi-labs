...
bool RunDownloadMultithread(PieceStorage& pieces, const TorrentFile& torrentFile, const std::string& ourId, const TorrentTracker& tracker) {
    ...
    while (pieces.PiecesSavedToDiscCount() < PiecesToDownload) {
        ...
        std::cout << "There are " << pieces.PiecesInProgressCount() << " still waiting" << std::endl;
...
void TestTorrentFile(const fs::path& file, int percents, const fs::path& fileToSave) {
    ...
    const std::filesystem::path outputDirectory = fileToSave;   //PrepareDownloadDirectory(PeerId);
    PiecesToDownload = (size_t)(floor(double(torrentFile.pieceHashes.size())*double(percents)/double(100)));
...
int main(int argc, char* argv[]) {
    TestTorrentFile(argv[5],std::stoi(argv[4]),argv[2]);
    return 0;
}
